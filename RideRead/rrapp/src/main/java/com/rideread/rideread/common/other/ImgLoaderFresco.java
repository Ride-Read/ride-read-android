...
public class ImgLoaderFresco {
    private static final String TAG = "ImgLoaderFresco";
     ...
     */
    ImgLoaderFresco(ImgFrescoLoadBuilder frescoBuilder) {
        this.mSimpleDraweeView = frescoBuilder.mSimpleDraweeView;

        //初始化M层 用于初始化图片中包含的数据
        GenericDraweeHierarchyBuilder builderM = new GenericDraweeHierarchyBuilder(Utils.getAppContext().getResources());

        //请求参数 主要配置url 和C层相关
        ImageRequest request = ImageRequestBuilder.newBuilderWithSource(Uri.parse(frescoBuilder.mUrl)).setResizeOptions(frescoBuilder.mResizeOptions).build();
        ...
        if (frescoBuilder.mBitmapDataSubscriber != null) {
            ...
            DataSource<CloseableReference<CloseableImage>> dataSource = imagePipeline.fetchDecodedImage(request, mSimpleDraweeView.getContext());
     ...
     * @param builderC
     */
    private void setViewPerformance(ImgFrescoLoadBuilder frescoBuilder, GenericDraweeHierarchyBuilder builderM, PipelineDraweeControllerBuilder builderC) {
