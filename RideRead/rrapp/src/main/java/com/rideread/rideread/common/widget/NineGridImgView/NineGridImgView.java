...
public class NineGridImgView<T> extends ViewGroup {
    ...
    private NineGridImgViewAdapter<T> mAdapter;

    public NineGridImgView(Context context) {
        this(context, null);
    }

    public NineGridImgView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mImgDataList != null && mImgDataList.size() > 0) {
            if (mImgDataList.size() == 1 && mSingleImgSize != -1) {
                ...
                mImageViewList.get(0).getHierarchy().setActualImageScaleType(ScalingUtils.ScaleType.CENTER_CROP);
     ...
     * @param adapter 适配器
     */
    public void setAdapter(NineGridImgViewAdapter adapter) {
        mAdapter = adapter;
    }
